...
\begin{equation}
    \Delta\phi = \frac{4\pi n L_g}{\lambda} \underbrace{\left(1 - \frac{n^2}{2}\left[p_{12} - \nu(p_{11} + p_{12})\right]\right)}_{\text{photoelastic correction } \approx 0.78} \epsilon
    \label{eq:phase_strain}
\end{equation}

where:
\begin{itemize}
    \item $n \approx 1.468$ is the refractive index of the fiber core
    \item $L_g$ is the gauge length (spatial resolution)
    \item $\lambda \approx 1550$ nm is the laser wavelength
    \item $p_{11} \approx 0.121$, $p_{12} \approx 0.270$ are the photoelastic coefficients for silica
    \item $\nu \approx 0.17$ is Poisson's ratio of the fiber
\end{itemize}

The photoelastic term accounts for the change in refractive index due to strain, which reduces the effective sensitivity by approximately 22\% compared to a naive geometric estimate.
...
\begin{equation}
    V_R \approx \left(\frac{0.87 + 1.12\nu}{1 + \nu}\right) V_S
...
\textbf{Data provenance:} The PoroTomo project was a 2016 DOE-funded field experiment at Brady Hot Springs, Nevada. The sample bundle used here preserves realistic acquisition geometry from that deployment while serving as a compact, verifiable stand-in for larger public DAS archives.

\textbf{Why a sample bundle?} Public raw DAS repositories often distribute multi-GB to multi-TB HDF5/TDMS holdings (which is excellent for research but impractical to ship inside an application package). This project therefore provides a compact sample bundle with real acquisition geometry metadata to demonstrate end-to-end processing.
...
\subsubsection{Note on Spatial Oversampling}

The dataset uses a 10\,m gauge length with 1\,m channel spacing, which means \textbf{adjacent channels are not independent measurements}---they are spatially oversampled with significant overlap. Each channel effectively averages strain over a 10\,m window, but centers are spaced only 1\,m apart.

\textit{Implications for processing:}
\begin{itemize}
    \item Adjacent channels exhibit high spatial correlation by design
    \item SVD and TV denoising methods exploit this coherence for noise suppression
    \item Effective spatial resolution is limited by gauge length ($\sim$10\,m), not channel spacing
    \item This oversampling is common in operational DAS deployments to improve SNR
\end{itemize}

The processing pipeline in this report accounts for this correlation structure: SVD extracts spatially coherent modes, and TV denoising enforces piecewise smoothness consistent with the underlying physics.
...
\noindent\textit{Note: The following is conceptual pseudocode illustrating the federated architecture design, not executable implementation code.}

\begin{lstlisting}[caption={Federated learning pseudocode (design proposal)}]
# Server-side
...
\subsubsection{Regulatory and Practical Considerations}

\textit{Important caveat:} While the bandwidth savings are compelling, real-world safety-critical CO2 monitoring operations face additional constraints:

\begin{itemize}
    \item \textbf{Regulatory requirements:} Many jurisdictions require ``human-in-the-loop'' verification of raw waveforms for critical events (potential leakage, significant seismicity). A system transmitting only model weights may not meet current regulatory standards for verifiable monitoring.

    \item \textbf{Hybrid architectures:} A practical deployment would likely use a tiered approach: (1) continuous local processing with FL-based anomaly detection, (2) selective raw waveform transmission for flagged events, and (3) periodic bulk transfers for archival and audit purposes.

    \item \textbf{Liability and auditability:} Operators may need to retain raw data locally even if not transmitted, to support post-incident analysis and regulatory audits.
\end{itemize}

The FL architecture presented here is a \textbf{design proposal} for bandwidth-constrained scenarios, not a complete regulatory-compliant system. Integration with existing monitoring frameworks and regulatory consultation would be required for operational deployment.
...
\section{Technical Notes and Assumptions}
\label{app:technical_notes}

This appendix documents key technical assumptions and implementation details for transparency.
...
\begin{table}[H]
    ...
    \begin{tabular}{lll}
        ...
        \midrule
        Data loading (NPZ/HDF5) & Concrete & Fully implemented \\
        Bandpass filtering & Concrete & Uses scipy.signal \\
        F-K filtering & Concrete & Uses numpy.fft \\
        SVD denoising & Concrete & Uses numpy.linalg \\
        TV denoising (ADMM) & Concrete & Custom implementation \\
        STA/LTA detection & Concrete & Custom implementation \\
        Federated Learning & \textbf{Conceptual} & Design proposal only \\
        \bottomrule
    \end{tabular}
\end{table}

The Federated Learning components (Listings in Appendix D) are \textbf{pseudocode} illustrating the architectural design. Unlike the signal processing code (which uses concrete scipy/numpy implementations), the FL code uses placeholder methods and would require a deep learning backend (PyTorch/TensorFlow) for operational deployment.

\subsection{Key Assumptions}

\begin{enumerate}
    \item \textbf{Isotropic media:} Rock physics equations (Gassmann, $V_P/V_S$ relations) assume isotropic elasticity. Real reservoirs exhibit anisotropy that affects wave velocities.

    \item \textbf{Plane-wave approximation:} The strain-rate to particle-velocity relation (Eq.~\ref{eq:strain_velocity_plane}) assumes 1D plane-wave propagation along the fiber.

    \item \textbf{Convex optimization:} ADMM convergence guarantees assume convex objectives with exact subproblem solves; practical implementations use finite-precision arithmetic.

    \item \textbf{Spatial oversampling:} The dataset's 10\,m gauge length with 1\,m channel spacing creates correlated channels (see Section~\ref{sec:data}).
\end{enumerate}

\subsection{Validation Approach}

The pipeline is validated through:
\begin{itemize}
    \item \textbf{Reproducibility:} All figures generated from the same sample data via \texttt{analysis\_real\_data\_report.py}
    \item \textbf{Parameter sensitivity:} STA/LTA detector validated across parameter grid (Figure~\ref{fig:report_stalta_sens})
    \item \textbf{Spectral consistency:} PSD comparisons between noise and event windows (Figure~\ref{fig:report_psd})
    \item \textbf{Visual QC:} Waterfall plots and F-K spectra for sanity checking
\end{itemize}

This approach prioritizes transparency and reproducibility over performance claims on benchmark datasets.
